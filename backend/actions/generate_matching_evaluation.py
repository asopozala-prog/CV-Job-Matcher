# Generate a structured candidate-to-role matching evaluation.
...
PROMPT_PATH = PROJECT_ROOT / "schemas" / "02matching_evaluation.md"
TASK_GROUP = "matching_evaluation"
...
def _parse_json_object(response: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError(
            "Gemini returned invalid JSON for matching evaluation: "
...
def _build_prompt(
    ...
    """Append hiring criteria and candidate material to the contract."""
    contract = _read_utf8(PROMPT_PATH, "Matching evaluation prompt")
    return (
        f"{contract.rstrip()}\n\n"
        "## INPUT\n\n"
        "### JOB OFFER ANALYSIS\n"
        f"{_json_object_text(hiring_criteria)}\n\n"
        "### CANDIDATE CV / CAREER MATERIAL\n"
...
def generate_matching_evaluation(
    hiring_criteria: Mapping[str, Any] | str,
    candidate_career_material_text: str,
    ...
    *,
    ...
    """Generate a matching evaluation, optionally write it, and return it."""
...
def main() -> None:
    """Run the matching action from local UTF-8 input files."""
    parser = argparse.ArgumentParser(description="Generate a Kiron matching evaluation.")
    ...
    result = generate_matching_evaluation(
